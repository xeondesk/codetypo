...
def test_command(tmp_path: Path) -> None:
    """Test running the codetypo executable."""
    # With no arguments does "."
    assert run_codetypo(cwd=tmp_path) == 0
    (tmp_path / "bad.txt").write_text("abandonned\nAbandonned\nABANDONNED\nAbAnDoNnEd")
    assert run_codetypo(cwd=tmp_path) == 4
...
def test_basic(
    ...
    with fname.open("w") as f:  # overwrite the file
        f.write("var = 'nwe must check codetypo likes escapes nin strings'\n")
    ...
    with fname.open("w") as f:  # overwrite the file
        f.write("var = '\\nwe must check codetypo likes escapes \\nin strings'\n")
...
@pytest.mark.parametrize(
    ...
    [
        # recommended form
        ("abandonned abondon abilty  # codetypo:ignore abondon", 2),
        ("abandonned abondon abilty  // codetypo:ignore abondon,abilty", 1),
        (
            "abandonned abondon abilty  /* codetypo:ignore abandonned,abondon,abilty",
            0,
        ),
        # ignore unused ignore
        ("abandonned abondon abilty  # codetypo:ignore nomenklatur", 3),
        # wildcard form
        ("abandonned abondon abilty  # codetypo:ignore ", 0),
        ("abandonned abondon abilty  # codetypo:ignore", 0),
        ("abandonned abondon abilty  # codetypo:ignore\n", 0),
        ("abandonned abondon abilty  # codetypo:ignore\r\n", 0),
        ("abandonned abondon abilty  # codetypo:ignore  # noqa: E501\n", 0),
        ("abandonned abondon abilty  # codetypo:ignore # noqa: E501\n", 0),
        ("abandonned abondon abilty  # codetypo:ignore# noqa: E501\n", 0),
        ("abandonned abondon abilty  # codetypo:ignore, noqa: E501\n", 0),
        ("abandonned abondon abilty  #codetypo:ignore\n", 0),
        # ignore these for safety
        ("abandonned abondon abilty  # codetypo:ignorenoqa: E501\n", 3),
        ("abandonned abondon abilty  codetypo:ignore\n", 3),
        ("abandonned abondon abilty codetypo:ignore\n", 3),
        # ignore these as they aren't valid
        ("abandonned abondon abilty  # codetypo:igore\n", 4),
        ...
        # prose examples
        (
            "You could also use line based igore ( codetypo:ignore ) to igore ",
            0,
        ),
        ("You could also use line based igore (codetypo:ignore) to igore ", 0),
        (
            "You could also use line based igore (codetypo:ignore igore) to igore ",
            0,
        ),
        (
            "You could also use line based igore (codetypo:ignore igare) to igore ",
            2,
        ),
    ],
)
...
def test_ignore_multiline_regex_option(
    ...
    assert (
        cs.main(
            ...
            "codetypo:ignore-begin.*codetypo:ignore-end",
        )
        == 2
    )
...
def test_quiet_level_32(
    ...
    with open(conf, "w", encoding="utf-8") as f:
        # It must contain a "codetypo" section.
        f.write("[codetypo]\n")
    args = ("--config", conf)
...
def run_codetypo_stdin(
    text: str,
    args: Tuple[Any, ...],
    cwd: Optional[Path] = None,
) -> int:
    """Run codetypo in stdin mode and return number of lines in output."""
    proc = subprocess.run(  # noqa: S603
        ["codetypo", *args, "-"],  # noqa: S607
        cwd=cwd,
        input=text,
        capture_output=True,
        encoding="utf-8",
